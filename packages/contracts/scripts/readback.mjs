...
import { canonicalContractSource, validateManifest } from './deployment-lib.mjs';
...
const code = await client.getContractCode(manifest.contractAddress);
const text = typeof code === 'string' ? code : new TextDecoder().decode(code);
const hash = `sha256:${createHash('sha256').update(canonicalContractSource(text)).digest('hex')}`;
if (hash !== manifest.sourceHash) throw new Error(`Deployed source mismatch: ${hash} != ${manifest.sourceHash}`);
const schema = await client.getContractSchema(manifest.contractAddress);
